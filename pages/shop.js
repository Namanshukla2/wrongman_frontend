...
export default function Shop() {
  ...
  const subCategoriesForCurrentCat = (() => {
    ...
    const set = new Set();
    prodsInCat.forEach(p => { if (p.subCategory && p.subCategory.trim()) set.add(p.subCategory.trim()); });
  ...
  useEffect(() => {
    ...
    if (q) {
      ...
      r = r.filter(p => p.name.toLowerCase().includes(qLower) || (p.brand && p.brand.toLowerCase().includes(qLower)));
    }
    if (sub) {
    ...
    switch (sort) {
      case 'price-low': r.sort((a, b) => (a.salePrice || a.price) - (b.salePrice || b.price)); break;
      case 'price-high': r.sort((a, b) => (b.salePrice || b.price) - (a.salePrice || a.price)); break;
      case 'newest': r.sort((a, b) => b.id - a.id); break;
      default: r.sort((a, b) => (b.featured ? 1 : 0) - (a.featured ? 1 : 0));
    }
    setFiltered([...r]);
      ...
      <div className="min-h-screen pt-20 skull-pattern" style={{ background: 'var(--bg)' }}>
        <div className="bg-red-600 py-10 px-4">
          <div className="max-w-7xl mx-auto">
            <h1 className="font-bebas text-4xl md:text-5xl text-white">TOP BRANDS, BEST PRICES ☠️</h1>
            <p className="text-white/80">All your favorite brands at killer prices!</p>
          </div>
        </div>
        ...
        <div className="max-w-7xl mx-auto px-4 py-8">
          <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4 mb-4">
              ...
              {cat !== 'all' && subCategoriesForCurrentCat.length > 0 && (
                <div className="flex flex-wrap gap-2 mt-3">
                  ...
                  {subCategoriesForCurrentCat.map(subName => (
                    <button key={subName} onClick={() => setSub(subName)}
            ...
            <div className="flex gap-3 w-full lg:w-auto">
              ...
              <select value={sort} onChange={e => setSort(e.target.value)} className="rounded-xl px-3 py-2 text-sm"
                style={{ background: 'var(--bg-3)', border: '1px solid var(--border)', color: 'var(--fg)' }}>
          ...
          <p className="mb-6 text-sm" style={{ color: 'var(--muted)' }}>{filtered.length} products found</p>

          {filtered.length === 0 ? (
            <div className="text-center py-16">
              <Skull className="w-16 h-16 mx-auto mb-4" style={{ color: 'var(--border)' }} />
              <p style={{ color: 'var(--muted)' }}>No products found</p>
              <button onClick={() => { setQ(''); setCat('all'); setSub(''); }} className="text-red-600 hover:underline mt-2">Clear filters</button>
            </div>
          ) : (
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 md:gap-6">
              {filtered.map(p => {
                ...
                return (
                  <div key={p.id} className="group rounded-2xl overflow-hidden border-2 hover:border-red-600 transition-all"
                    ...
                    <Link href={`/product/${p.id}`} className="block relative aspect-[3/4] overflow-hidden">
                      <img src={isHovered && hoverImage ? hoverImage : mainImage} alt={p.name}
                    ...
                    <div className="p-4">
                      ...
                      <button onClick={() => add(p)} className="w-full bg-red-600 hover:bg-red-700 text-white font-bold py-2.5 rounded-xl text-sm">
                        Add to Cart
                      </button>
                    </div>
                  </div>
